...
from bmp.domain.packing.packing import DomainPacking, ProblemPacking
from bmp.domain.kitchen.kitchen import DomainKitchen, ProblemKitchen
from bmp.domain.unpacking.unpacking import DomainUnpacking, ProblemUnpacking
from pddlstream.algorithms.downward import Problem
...
def pddlstream_from_problem(prob:ProblemPacking)->PDDLProblem:
    domain = prob.domain
    ...
    # for movable in movables:
    #     goal += [("On", movable, goal_placeable)]
    goal = (
        "and",
        ("AtAConf", panda, config_init),
...
def main():
    ...
    verbose = True
    pddlstream_problem = pddlstream_from_problem(prob)
    stream_info = {
        'inverse-kinematics': StreamInfo(),
        'plan-arm-motion': StreamInfo(overhead=1e1),
    ...
    with Profiler(field='tottime', num=100): # cumtime | tottime
        solution = solve(
            pddlstream_problem, 
            algorithm="adaptive", 
...
def visualize(prob:ProblemPacking, plan):
