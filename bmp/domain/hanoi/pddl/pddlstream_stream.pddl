(define (stream pr2-tamp)
  (:stream sample-pose
    :inputs (?o ?r ?rp) ;
    :domain (and (Stackable ?o ?r) (Pose ?r ?rp)) ;
    :outputs (?p)
    :certified (and (Pose ?o ?p) (Supported ?o ?p ?r ?rp)) ; 
  )
  (:stream sample-grasp
    :inputs (?o)
    :domain (Graspable ?o)
  ...
  (:stream inverse-kinematics
    :inputs (?a ?o ?p ?g ?r ?rp)
    :domain (and (Arm ?a) (Pose ?o ?p) (Grasp ?o ?g) (Supported ?o ?p ?r ?rp))
    :outputs (?q ?t)
    :certified (and (AConf ?q) (ATraj ?t) (Kin ?a ?o ?p ?g ?r ?rp ?q ?t)) ; 
  )
