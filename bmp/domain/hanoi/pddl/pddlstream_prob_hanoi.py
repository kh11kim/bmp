...
def pddlstream_from_problem(domain_pddl_path, stream_pddl_path, prob:ProblemHanoi)->PDDLProblem:
    ...
    for movable in movables:
        ...
        if movable.name in mode.attachments:
            ...
            if att.parent_name in mode.attachments:    
                ...
                init += [('Disc-upported', movable, att, parent, att2)] #, 
                init += [('On-disc', movable, parent)]
            else:
                init += [('Peg-supported', movable, att, parent)] #, 
                init += [('On-peg', movable, parent)]
    ...
    init += [
        ("Smaller", movables['disk1'], movables['disk2']),
        ("Smaller", movables['disk1'], movables['disk3']),
        ("Smaller", movables['disk2'], movables['disk3']),
        
        ("Clear", movables['disk1']),
        ("Empty-peg", regions['peg2']),
        ("Empty-peg", regions['peg3']),
    ]

    goal = [
        "and",
        ("AtAConf", panda, config_init),
        ("On-disc", movables['disk1'], movables['disk2']),
        ("On-disc", movables['disk2'], movables['disk3']),
        ("On-peg", movables['disk3'], regions['peg3'])#
    ]
    
    stream_map = {
        'sample-peg-place-pose': from_gen_fn(get_stable_gen(domain)),
        'sample-disc-place-pose': from_gen_fn(get_stable_gen2(domain)),
