...
def pddlstream_from_problem(domain_pddl_path, stream_pddl_path, prob:ProblemNonmonotonic, num_box=3)->PDDLProblem:
    domain = prob.domain
    ...
    for movable in movables:
        ...
        placement = mode.attachments[movable.name]
        init += [('Pose', movable, placement)]
        init += [('AtPose', movable, placement)]
        init += [('Supported', movable, placement, domain.regions[placement.parent_name])]
    ...
    goal = [
        "and",
        ("AtAConf", panda, config_init),
        ("On", movables["red1"], regions["plate_red"]),
        ("On", movables["green1"], regions["plate_green"]),
        ("On", movables["blue1"], regions["plate_blue"]),
    ]
...
def pddlstream_solve(prob, timeout=100):
    planner = "ff-astar"
    solution = solve(
        prob, 
        algorithm="adaptive", 
